...
# raises UnicodeEncodeError on the non-ASCII characters used in this module's output
# (e.g. the "->" arrow below). Force UTF-8 so the CLI works without PYTHONUTF8=1.
...
def _match_known_id(normalized: str, known_story_ids: set[str]) -> str | None:
    for story_id in known_story_ids:
        numeric_prefix = "-".join(story_id.split("-", 2)[:2])
        if normalized == story_id.lower() or normalized == numeric_prefix.lower():
            return story_id
    return None


def _story_refs(text: str, known_story_ids: set[str]) -> set[str]:
    refs: set[str] = set()
    for raw in STORY_REF.findall(text):
        normalized = raw.lower().replace(".", "-")
        match = _match_known_id(normalized, known_story_ids)
        if match:
            refs.add(match)
    return refs
...
def extract_dependencies(
    story_text: str, known_story_ids: set[str], *, self_id: str | None = None
) -> tuple[str, ...]:
    """Extract explicit, direction-unambiguous story prerequisites from markdown.
    ...
    *1.6* depends on the story being read, not the reverse, but a naive scanner
    ...
    for line in story_text.splitlines():
        ...
        if depends_col is None:
            ...
            continue
        if depends_col >= len(cells):
            continue
        cell = cells[depends_col]
        if REVERSE_DIRECTION.search(cell):
            continue
        found |= _story_refs(cell, known_story_ids)
