"""LLMClient — shared LLM call resilience layer (extracted in Story 2.2).

Transforms a serialized :class:`InsightPayload` JSON string into an
:class:`InsightReport` (structured prose) using Claude Structured Outputs.
Extracted verbatim from ``backend/pipeline/narrative_generator.py`` so the
Reporting Agent (2.3) and Monitoring Agent (2.4) can invoke LLM calls
without duplicating this machinery.
...
This is an internal code-reuse module, NOT an abstraction layer: provider
SDKs stay as lazy imports inside their ``_call_*`` methods, callers still
pass JSON and receive an ``InsightReport``, and no routing framework is
involved (architecture.md §289).
"""
...
class LLMClient:
    """Resilient LLM caller: retry, provider fallback, circuit breaker."""

    def __init__(self) -> None:
        # stage="narrative_generator" is load-bearing: monitoring and
        # alerting read this key from the JSON log stream. Do not rename
        # it to match the module.
