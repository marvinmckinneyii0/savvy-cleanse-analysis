"""LLMClient — resilient LLM narrative generation, extracted from Stage 3.

Applies the full resilience chain to turn a computed-statistics payload
into narrated prose via Claude Structured Outputs, with fallback to
OpenAI and Gemini:
...
Extracted in Story 2.2 from ``backend/pipeline/narrative_generator.py`` so
the Reporting Agent (2.3) and Monitoring Agent (2.4) can reuse this
machinery without duplicating it. This is an internal code-reuse module,
not a provider-abstraction framework — see Story 2.2 Dev Notes for why
that distinction matters against the "no abstraction layers" architecture
constraint. Provider SDKs (``anthropic``, ``openai``, ``google.genai``)
remain lazy imports inside their respective ``_call_*`` methods.
"""
...
class LLMClient:
    """Resilient LLM narrative generation shared across pipeline agents."""

    def __init__(self) -> None:
        self._logger = structlog.get_logger().bind(stage="narrative_generator")

    def generate_narrative(self, payload_json: str, pipeline_run_id: str) -> InsightReport:
        """Transform a computed-statistics JSON payload into a narrated InsightReport.
