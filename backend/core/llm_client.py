"""LLMClient — shared LLM call resilience for pipeline stages and agents.

Extracted from ``backend/pipeline/narrative_generator.py`` in Story 2.2 so
the Reporting Agent (2.3) and Monitoring Agent (2.4) can invoke LLM calls
without duplicating this machinery. This is an internal code-reuse module,
not an abstraction layer — provider SDKs stay as lazy imports inside their
``_call_*`` methods, and callers still pass JSON and get an
:class:`InsightReport` (architecture.md §289: no LangChain, no external
routing layers).
...
class LLMClient:
    """Retry / fallback / circuit-breaker wrapper around the LLM providers.

    Instantiate one per caller (``NarrativeGenerator``, future agents) —
    matching the per-instance pattern used across the pipeline; this is
    deliberately not a module-level singleton.
    """

    def __init__(self) -> None:
        # stage="narrative_generator" is load-bearing: monitoring reads the
        # JSON log stream and this binding predates the extraction (AC6).
    ...
    def generate_narrative(
        ...
        """Narrate a serialized InsightPayload into an InsightReport.
        ...
        ``pipeline_run_id`` identifies the run for callers' correlation;
        binding it into the structlog context
        (:func:`backend.pipeline.config.bind_pipeline_run_id`) is the
        caller's responsibility — this module deliberately does not import
        from ``backend.pipeline``.
        """
        del pipeline_run_id  # correlation is bound by the caller (see docstring)
