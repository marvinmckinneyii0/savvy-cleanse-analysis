...
Story 1.1 installed the logging hooks below. The config schema was
defined in Story 2.1 — see :mod:`backend.models.pipeline_config`, where
the Pydantic :class:`PipelineConfig` contract lives alongside the other
model-layer contracts. It is re-exported here so the historical import
path ``from backend.pipeline.config import PipelineConfig`` keeps
working.
