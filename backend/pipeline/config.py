...
Story 1.1 installs the logging hooks. The config schema is defined in
Story 2.1: the Pydantic contract lives in
:mod:`backend.models.pipeline_config` (the models layer, alongside every
other pipeline contract) and :class:`PipelineConfig` is re-exported here
so ``from backend.pipeline.config import PipelineConfig`` keeps working.
