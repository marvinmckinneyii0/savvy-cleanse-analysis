...
- Baseline before implementation: `uv run pytest backend/tests/` → 123 passed, 1 skipped
  (pre-existing skip: anthropic SDK not installed in dev env).
- After implementation: 152 passed, 1 skipped — 29 new tests in `test_config.py`, zero regressions.

### Completion Notes List

- Followed the prescribed location resolution exactly: Pydantic models live in
  `backend/models/pipeline_config.py`; `backend/pipeline/config.py` re-exports `PipelineConfig`
  (both import paths resolve to the same class — covered by a test). `configure_logging()` and
  `bind_pipeline_run_id()` untouched.
- `metric_thresholds` is a flat `dict[str, float]` (matches the canonical YAML shape) with a
  `threshold_for(metric)` accessor that falls back to `DEFAULT_THRESHOLD = 0.15` — the generic
  per-column surface 2.2/2.4 need. Values validated `> 0`.
- `ScheduleConfig` enforces exactly-one-of interval/cron; cron validated via `croniter.is_valid`.
- Secrets: `load()` calls `load_dotenv()` then builds `SmtpSettings.from_env()`; the loader
  additionally REJECTS a config.yaml containing secret keys (`smtp`, `smtp_password`, …) with a
  descriptive `ConfigurationError`. Validation-error messages are built with
  `errors(include_input=False)` so secret values can never echo into exception text.
- `load()` is stateless (re-read + re-validate every call) and the model is frozen, so a previously
  returned instance is an immutable snapshot while a re-load reflects on-disk edits (AC5).
- `config_loaded` structlog event logs schedule, threshold keys, recipient COUNT (PII-adjacent —
  count only), output format, and `smtp_configured` boolean — asserted secret-free in tests.
- `extra="forbid"` on all models so config typos fail loudly instead of being silently ignored.
- Dependency note: the four config deps were added to BOTH `pyproject.toml` and
  `backend/requirements.txt` per Task 4; pre-existing drift between the two files left as-is.

### File List

- `backend/models/pipeline_config.py` (new) — Pydantic config contract + `PipelineConfig.load()`
- `backend/tests/test_config.py` (new) — 29 tests covering AC 1–6
- `config.yaml` (new, repo root) — documented canonical example, no secrets
- `backend/pipeline/config.py` (modified) — stub dataclass → re-export; stale docstring updated
- `.env.example` (modified) — SMTP placeholder block added
...
- `uv.lock` (modified) — lockfile refresh for the new deps

## Code Review Record (fresh-perspective pass, 2026-07-06)

Independent review verified all 6 ACs and the Dev Notes constraints (safe_load only, layer
boundaries, re-export, logging helpers untouched). **No Critical or High findings** → security
review not triggered per sprint policy. Findings addressed before closure:

- **Medium** `UnicodeDecodeError` escaped `load()` on a non-UTF-8 config file → now wrapped in
  `ConfigurationError` (+ regression test).
- **Medium** `test_smtp_defaults_when_env_absent` was flaky on machines with a real `.env` (dotenv
  would repopulate deleted vars; a failure would echo real SMTP creds into pytest output) →
  dotenv neutralized via monkeypatch in that test.
- **Low** no-arg `load_dotenv()` walks above the repo root and could load a foreign `.env` →
  now `load_dotenv(_PROJECT_ROOT / ".env")`, deterministic.
- **Low** chained `ValidationError` repr echoed input values (recipient emails are PII-adjacent)
  into tracebacks → `raise ... from None`; the summary message still names every offending field.
- **Low** `test_validation_error_does_not_escape` passed vacuously if nothing raised → `else:
  pytest.fail(...)` added.
- **Low** AC3 default-path branch untested → read-only no-arg `load()` test added (also guards
  the committed repo-root `config.yaml` against future breakage).
- **Nit** dead `_SMTP_ENV_VARS` constant removed; empty-config-file case now gets a clear
  "config file is empty" message (+ test). Also fixed during dev: `IsADirectoryError`/
  `PermissionError` now wrapped via an `OSError` clause (+ test).
- Accepted as-is: `frozen=True` is shallow (documented behavior, reload semantics unaffected);
  croniter accepts 6/7-field cron — noted as a 2.3 forward-compat concern.

Post-review suite: **156 passed, 1 skipped** (pre-existing skip), zero regressions.
