...
Claude Fable 5 (Claude Code, scheduled automation run 2026-07-05)

### Debug Log References

- Baseline before implementation: `pytest backend/tests/` — 123 passed, 1 skipped (anthropic SDK absent in env).
- After implementation: 146 passed, 1 skipped — 23 new tests in `test_config.py`, zero regressions.
- Smoke test: `PipelineConfig.load()` against the real repo-root `config.yaml` emits the JSON
  `config_loaded` event (schedule=weekly, threshold_keys=[revenue, units_sold], recipient_count=1).

### Completion Notes List

- Followed the prescribed location resolution exactly: Pydantic models in
  `backend/models/pipeline_config.py`, `load()` classmethod on `PipelineConfig`, re-export from
  `backend/pipeline/config.py` (logging functions untouched).
- Schema shape follows the canonical `config.yaml` in Dev Notes: `data_sources` and
  `alert_recipients` are plain top-level lists (validated fields on the root model, since a wrapper
  model cannot map to a top-level YAML list); `report_schedule` and `output` are nested models
  (`ScheduleConfig`, `OutputConfig`); thresholds use the sanctioned `dict[str, float]` + validator
  alternative with `DEFAULT_THRESHOLD = 0.15` and a `threshold_for()` helper for 2.2's per-column reads.
- Secrets discipline: `SmtpSettings.from_env()` is the only SMTP source; an `smtp:` key in YAML is
  rejected with `ConfigurationError`. `ValidationError` re-raise emits field loc + msg only (never
  input values) so env-sourced values cannot leak into exception text.
- `extra="forbid"` on all models so config typos fail loudly with the offending key named.
- Dependencies added to `pyproject.toml` + `uv.lock` (via `uv add`) and mirrored in
  `backend/requirements.txt`: pyyaml>=6.0, croniter>=2.0, email-validator>=2.0, python-dotenv>=1.0
  (dotenv was the pre-existing pyproject gap). Pre-existing drift between the two files left as-is
  per Task 4.
- Security review (DoD gate): zero Critical/High findings. safe_load-only confirmed; no secrets in
  YAML, logs, or exception messages. Low/informational note for 2.4: consider `SecretStr` for
  `SmtpSettings.password` when alert delivery starts consuming it.
...
- `config.yaml` (new)
- `backend/pipeline/config.py` (modified — stub replaced with re-export, docstring updated)
- `.env.example` (modified — SMTP placeholders)
- `pyproject.toml` / `uv.lock` (modified — 4 config deps)
- `backend/requirements.txt` (modified — same deps mirrored)
